...
const QA_ISSUE_KEY_PREFIX = "factory_issue_" as const;

function QAIssueLabel({ code, t }: { code: string; t: (k: string) => string }) {
  // Strip dynamic suffix (e.g. "manifest_scene_mismatch:expected=3,got=2")
  const base = code.split(":")[0];
  const key = `${QA_ISSUE_KEY_PREFIX}${base}` as any;
                ...
                {publishData.status && (
                  <div className="rounded-3xl border border-white/10 bg-white/5 p-4 space-y-2">
                    ...
                    <dl className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                      ...
                      <dd className="text-white/70">
                        {publishData.dry_run
                          ? t("factory_publish_dry_run_label")
                          : t("factory_publish_live_label")}
                      </dd>
                    </dl>
                    {publishData.dry_run && (
