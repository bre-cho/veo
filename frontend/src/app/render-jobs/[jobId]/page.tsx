...
import { acknowledgeRenderIncident, assignRenderIncident, getRecentRenderIncidents, getRenderJob, getRenderJobEvents, getRenderJobHealth, getRenderSceneEvents, muteRenderIncident, decideRebuild, approveRebuild, type RecentIncidentItem, type RenderEventItem, type RenderJob, type RenderJobHealthSummary } from "@/src/lib/api";
...
export default function RenderJobPage() {
  ...
  const handleGetRebuildDecision = async () => {
    if (!rebuildEpisodeId || !rebuildSceneId || !job?.project_id) return;
  ...
  const handleApproveRebuild = async (decision: any) => {
    try {
      const result = await approveRebuild(decision, jobId ?? undefined);
      ...
      <section className="mx-auto max-w-5xl space-y-4 px-4 py-6">
        <div className="rounded-3xl border border-white/10 bg-white/5 p-5">
          ...
          <div className="mb-4 grid gap-3 sm:grid-cols-3">
            ...
            />
            <select
              value={rebuildChangeType}
              onChange={(e) => setRebuildChangeType(e.target.value)}
              className="rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-sm text-white outline-none"
            >
          ...
          <button
            ...
            disabled={rebuildLoading || !rebuildEpisodeId || !rebuildSceneId || !job?.project_id}
            className="rounded-xl border border-white/15 bg-white/10 px-4 py-2 text-sm font-medium text-white transition hover:border-white/30 disabled:opacity-50"
          >
        ...
        {rebuildExecuteResult && (
          <div className="rounded-3xl border border-white/10 bg-white/5 p-5 text-xs text-white/60">
            <strong className="text-white/80">Execute result:</strong>{" "}
            {JSON.stringify(rebuildExecuteResult)}
          </div>
        )}
      </section>
    </>
