...
# ---------------------------------------------------------------------------
# Adaptive scoring: performance_boost parameters
# ---------------------------------------------------------------------------
# Baseline score when no performance data exists
_ADAPTIVE_BASE_SCORE = 0.5
# Maximum boost a high-performing template_family/hook can add
_MAX_PERFORMANCE_BOOST = 0.25
# Minimum records required to apply a performance boost
_BOOST_MIN_RECORDS = 3
...
class PerformanceLearningEngine:
    ...
    **dual-writes** every record to the ``performance_records`` table so ops
    ...
    Standardised event schema (Sprint 5):
        {
            "video_id": str,
            "avatar_id": str | null,
            "product_id": str | null,
    ...
    def record(
        ...
        *,
        ...
        # Sprint 5: extended event schema
        avatar_id: str | None = None,
        product_id: str | None = None,
        experiment_id: str | None = None,
        variant_id: str | None = None,
        performance_metrics: dict[str, Any] | None = None,
        # Phase 1.1: persona / category / funnel dimensions
    ...
    def record_experiment(
        ...
        *,
        ...
        """Record a performance event tied to an experiment + variant.

        Automatically generates stable IDs from experiment_name / variant_name
        using UUID5 (deterministic, namespace-free).
        """
        experiment_id = str(uuid.uuid5(uuid.NAMESPACE_DNS, f"exp:{experiment_name}"))
        variant_id = str(uuid.uuid5(uuid.NAMESPACE_DNS, f"var:{experiment_name}:{variant_name}"))
        return self.record(
            video_id=video_id,
            hook_pattern=hook_pattern,
            cta_pattern=cta_pattern,
    ...
    def adaptive_score(
        ...
        *,
        ...
        - ``base`` is always ``_ADAPTIVE_BASE_SCORE`` (0.5)
        - ``performance_boost`` is derived from time-weighted historical performance
          for this specific template_family (and optionally hook_pattern) in the
          given platform/market context.
        ...
        if hook_pattern:
            ...
            if len(hp_records) >= _BOOST_MIN_RECORDS:
    ...
    def experiment_summary(
        ...
        """Return a summary of variant performance for a given experiment."""
        if experiment_name and not experiment_id:
            experiment_id = str(uuid.uuid5(uuid.NAMESPACE_DNS, f"exp:{experiment_name}"))
        records = [r for r in self._records if r.get("experiment_id") == experiment_id]
        if not records:
            return {"experiment_id": experiment_id, "variants": [], "winner_variant_id": None}

        variant_data: dict[str, list[float]] = {}
        for r in records:
            vid = r.get("variant_id") or "unknown"
            variant_data.setdefault(vid, []).append(float(r["conversion_score"]))
        ...
        variants.sort(key=lambda v: v["avg_score"], reverse=True)
    ...
    def persona_score_matrix(
        ...
        *,
        ...
        """Return a {persona_id → {hook_pattern → avg_score}} matrix.

        Only personas with at least ``min_samples`` records are included.
        Scores are time-weighted averages (90-day half-life).
        """
        ...
        for pid, persona_records in by_persona.items():
            if len(persona_records) < min_samples:
                continue
            hook_scores: dict[str, list[tuple[float, float]]] = {}
            for r in persona_records:
                hp = r.get("hook_pattern", "")
                if not hp:
                    continue
                w = _time_weight(float(r.get("recorded_at") or time.time()))
                hook_scores.setdefault(hp, []).append((float(r["conversion_score"]), w))
            if not hook_scores:
                continue
            pattern_avg: dict[str, float] = {}
            for hp, pairs in hook_scores.items():
                total_w = sum(ww for _, ww in pairs)
                if total_w < 1e-9:
                    continue
                pattern_avg[hp] = round(sum(s * w for s, w in pairs) / total_w, 3)
            if pattern_avg:
                matrix[pid] = dict(
                    sorted(pattern_avg.items(), key=lambda kv: kv[1], reverse=True)
                )
        return matrix
    ...
    def _filter_records(
        ...
        *,
        ...
        if persona_id:
            # persona_id can match either the explicit persona_id field or
            # the avatar_id field (avatar acts as a persona proxy).
            records = [
                r for r in records
                if r.get("persona_id") == persona_id or r.get("avatar_id") == persona_id
            ]
        if product_category:
            records = [r for r in records if r.get("product_category") == product_category]
        return records
    ...
    def _db_upsert(self, rec: dict[str, Any]) -> None:
        ...
        try:
            ...
            if existing is not None:
                ...
                existing.persona_id = rec.get("persona_id")
                existing.product_category = rec.get("product_category")
                existing.funnel_stage = rec.get("funnel_stage")
                existing.campaign_id = rec.get("campaign_id")
