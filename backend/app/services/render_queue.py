...
def enqueue_render_identity_review(job_id: str, countdown: int = 0) -> dict:
    if countdown > 0:
        result = render_identity_review_task.apply_async(args=[job_id], countdown=countdown)
    else:
        result = render_identity_review_task.delay(job_id)

    return {
        "task_name": "render.identity_review",
        "celery_task_id": result.id,
        "job_id": job_id,
        "countdown": countdown,
    }
