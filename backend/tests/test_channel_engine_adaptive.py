...
def _make_learning_store(
    ...
    tmp_path: str = "/tmp/test_channel_adaptive_store.json",
) -> PerformanceLearningEngine:
    eng = PerformanceLearningEngine(store_path=tmp_path)
    eng.clear()
    for i in range(n):
        eng.record(
            video_id=f"v{i}",
...
def test_no_adjustment_when_fewer_than_5_records() -> None:
    eng = _make_learning_store(n=3, tmp_path="/tmp/test_ch_adj_few.json")
    assert _derive_adaptive_weight_adjustments(eng) == {}


def test_high_conversion_boosts_conversion_potential() -> None:
    eng = _make_learning_store(n=10, conversion_score=0.9, tmp_path="/tmp/test_ch_adj_high.json")
    adj = _derive_adaptive_weight_adjustments(eng)
    assert adj.get("conversion_potential", 0) > 0


def test_low_conversion_boosts_audience_fit() -> None:
    eng = _make_learning_store(n=10, conversion_score=0.3, tmp_path="/tmp/test_ch_adj_low.json")
    adj = _derive_adaptive_weight_adjustments(eng)
    assert adj.get("audience_fit", 0) > 0
    assert adj.get("platform_fit", 0) > 0


def test_stable_hook_pattern_boosts_repeatability() -> None:
    eng = _make_learning_store(
        n=10, conversion_score=0.9, hook_pattern="hook-x",
        tmp_path="/tmp/test_ch_adj_hook.json",
    )
    adj = _derive_adaptive_weight_adjustments(eng)
    assert adj.get("repeatability", 0) > 0


def test_adjustments_clamped_to_max() -> None:
    from app.services.channel_engine import _MAX_WEIGHT_ADJUSTMENT

    eng = _make_learning_store(n=20, conversion_score=0.99, tmp_path="/tmp/test_ch_adj_clamp.json")
    adj = _derive_adaptive_weight_adjustments(eng)
    for v in adj.values():
...
def test_generate_plan_with_learning_store_marks_feedback_applied() -> None:
    engine = ChannelEngine()
    store = _make_learning_store(
        n=10, conversion_score=0.85, tmp_path="/tmp/test_ch_feedback.json"
    )
...
def test_generate_plan_produces_valid_series(tmp_path) -> None:
    engine = ChannelEngine()
    store = _make_learning_store(
        n=6, conversion_score=0.6, tmp_path=str(tmp_path / "store.json")
    )
...
def test_generate_plan_winner_has_highest_score() -> None:
    engine = ChannelEngine()
    store = _make_learning_store(
        n=8, conversion_score=0.4, tmp_path="/tmp/test_ch_winner.json"
    )
